...
<head>
    ...
    def get_report_header(
        title: str, subtitle: Optional[str] = None, nav_links: Optional[list] = None
    ) -> str:
        """Generate consistent report header."""
        subtitle_html = (
            f'<div class="nav-subtitle">{subtitle}</div>' if subtitle else ""
        )
        ...
        if nav_links:
            ...
            for link in nav_links:
                if isinstance(link, dict):
                    nav_items.append(
                        f'<a href="{link.get("href", "#")}" class="nav-link">{link.get("text", "")}</a>'
                    )
                else:
                    nav_items.append(
                        f'<a href="#{link.lower().replace(" ", "-")}" class="nav-link">{link}</a>'
                    )
