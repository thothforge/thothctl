...
class HTMLReportGenerator:
        ...
        for root, _, files in os.walk(abs_directory):
            for file in files:
                if file == "results_junitxml.xml":
                    ...
                    "_results_junitxml.xml"
                ):  # Also check for the old expected pattern
                    xml_path = os.path.join(root, file)
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Not a valid JUnit XML file: {xml_path} - {e}"
                        )

        if not xml_files:
            self.logger.warning(
                f"No XML files found in {abs_directory} or its subdirectories"
            )
            ...
            if all_xml_files:
                self.logger.info(
                    f"Found {len(all_xml_files)} XML files in directory (not JUnit format):"
                )
                for xml_file in all_xml_files:
                    self.logger.info(
                        f"  {xml_file} ({os.path.getsize(xml_file)} bytes)"
                    )

            # Check if there are any JSON files that might have been created
            json_files = glob.glob(
                f"{abs_directory}/**/*results_json.json", recursive=True
            )
            if json_files:
                self.logger.info(f"Found {len(json_files)} JSON result files:")
                for json_file in json_files:
                    self.logger.info(
                        f"  {json_file} ({os.path.getsize(json_file)} bytes)"
                    )
    ...
    def create_individual_reports(
        self, xml_files: List[str], output_dir: str
    ) -> List[Dict]:
        ...
        for xml_file in xml_files:
            try:
                ...
                if dir_name.startswith("report_"):
                    ...
                    file_name = os.path.basename(xml_file).replace(
                        "_results_junitxml.xml", ""
                    )
                ...
                for testsuite in root.findall(".//testsuite"):
                    ...
                    for testcase in testsuite.findall(".//testcase"):
                        ...
                        result = "passed"
                        message = ""
                        failure_details = {"resource": "", "file": "", "guideline": ""}
                        ...
                        if failure is not None:
                            ...
                            for line in failure_lines:
                                ...
                                if line.startswith("Resource:"):
                                    failure_details["resource"] = line.replace(
                                        "Resource:", ""
                                    ).strip()
                                elif line.startswith("File:"):
                                    failure_details["file"] = line.replace(
                                        "File:", ""
                                    ).strip()
                                elif line.startswith("Guideline:"):
                                    failure_details["guideline"] = line.replace(
                                        "Guideline:", ""
                                    ).strip()
                            ...
                            if (
                                not failure_details["resource"]
                                and len(failure_lines) > 0
                            ):
                ...
                self.logger.info(
                    f"Found {len(failed_checks)} failed checks in {xml_file}"
                )
                for i, check in enumerate(failed_checks):
                    self.logger.info(
                        f"Failed check {i + 1}: {check['check_id']} - {check['check_description']}"
                    )
                    self.logger.info(
                        f"  Resource: {check['failure_details']['resource']}"
                    )
                    self.logger.info(f"  File: {check['failure_details']['file']}")
                    self.logger.info(
                        f"  Guideline: {check['failure_details']['guideline']}"
                    )
                ...
                self.logger.error(
                    f"Error creating individual report for {xml_file}: {e}",
                    exc_info=True,
                )
        ...
        try:
            ...
            for xml_file in xml_files:
                try:
                    ...
                    for testsuite in root.findall(".//testsuite"):
                        ...
                        for testcase in testsuite.findall(".//testcase"):
                            ...
                            if failure is not None:
                                ...
                                message = failure.get("message", "")
                                failure_text = (
                                    failure.text.strip() if failure.text else ""
                                )
                    ...
                    results.append(
                        {"name": file_name, "suites": suites, "file": xml_file}
                    )
                except Exception as e:
                    self.logger.error(f"Error parsing XML file {xml_file}: {e}")
        ...
        try:
            ...
            for html_file in html_files:
                ...
                if validation["issues"]:
                    self.logger.warning(
                        f"Inconsistencies found in {html_file}: {validation['issues']}"
                    )
                    ...
                    if HTMLReportUtils.fix_report_consistency(html_file):
                        ...
                        self.logger.error(
                            f"Failed to fix consistency issues in {html_file}"
                        )
                else:
            ...
            if inconsistent_reports:
                print(
                    f"⚠️  Fixed styling inconsistencies in {len(inconsistent_reports)} reports"
                )
            else:
