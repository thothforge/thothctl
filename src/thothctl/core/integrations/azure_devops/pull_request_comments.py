...
def post_comment_to_azure_devops_pr(
    ...
    try:
        ...
        connection = Connection(base_url=f"https://dev.azure.com/{organization_name}", creds=credentials)
        ...
        created_thread = git_client.create_thread(thread, repository_id, pull_request_id, project)
        if created_thread:
            ...
            print(f"{Fore.RED}📛 Failed to post comment. The created_thread is None. {Fore.RESET}")
