...
def remove_space(space_name: str, remove_projects: bool = False):
    ...
    space_in_config = (
        "spaces" in spaces_config and space_name in spaces_config["spaces"]
    )
    ...
    if projects_in_space:
        if remove_projects:
            print(
                f"{Fore.YELLOW}Removing all projects in space '{space_name}'...{Fore.RESET}"
            )
            ...
            print(
                f"{Fore.YELLOW}Removing space association from projects...{Fore.RESET}"
            )
            for project_name in projects_in_space:
                if project_name in conf and "thothcf" in conf[project_name]:
                    if "space" in conf[project_name]["thothcf"]:
                        ...
                        print(
                            f"{Fore.GREEN}Removed space association from project '{project_name}'{Fore.RESET}"
                        )
    else:
    ...
    if "spaces" in spaces_config and space_name in spaces_config["spaces"]:
        ...
        print(
            f"{Fore.GREEN}Removed space '{space_name}' from spaces configuration.{Fore.RESET}"
        )
