...
class ProjectBootstrapService:
    ...
    def bootstrap_project(
        ...
        try:
            ...
            return {"success": True, "project_info": project_info, "changes": changes}
    ...
    def _check_scaffold_files(
        ...
        changes = {"new_files": [], "updated_files": [], "skipped": []}
    ...
    def _apply_bootstrap_changes(
        ...
        if changes.get("config_action"):
            self._handle_config_file(
                project_path, project_info, template_url, changes["config_action"]
            )
    ...
    def _handle_scaffold_files(
        self, project_path: Path, template_url: str, changes: Dict
    ) -> None:
    ...
    def _merge_amazonq_file(
        self, source_file: Path, target_file: Path, rel_path: str
    ) -> None:
    ...
    def _create_amazonq_file(self, file_path: Path, rel_path: str) -> None:
        ...
        if "project.json" in rel_path:
            ...
            content = {"conversations": [], "last_updated": datetime.now().isoformat()}
        elif "workspace_state.json" in rel_path:
            content = {
                "workspace": {
    ...
    def _create_basic_amazonq_files(
        self, project_path: Path, file_list: List[str]
    ) -> None:
