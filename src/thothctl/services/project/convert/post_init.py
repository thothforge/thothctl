...
def add_project_properties_to_thothcf(project_dir, project_properties):
    ...
    if "[project_properties]" in content:
        logging.info(
            f"{Fore.GREEN}project_properties already exists in {thothcf_file}{Fore.RESET}"
        )
        return
...
"""
    ...
    try:
        ...
        logging.info(
            f"{Fore.GREEN}Successfully added project_properties to {thothcf_file}{Fore.RESET}"
        )
    except Exception as e:
        logging.error(
            f"{Fore.RED}Error writing to {thothcf_file}: {str(e)}{Fore.RESET}"
        )
