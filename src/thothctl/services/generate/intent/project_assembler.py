...
When a ScaffoldStructure is provided, uses real boilerplate content from
the official scaffold instead of hardcoded templates.
"""
...
if TYPE_CHECKING:
    from .scaffold_loader import ScaffoldStructure
...
class ProjectAssembler:
    ...
    def assemble(
        ...
        scaffold: Optional["ScaffoldStructure"] = None,
    ) -> List[GeneratedFile]:
        """Assemble full project structure from composition plan.

        When a scaffold is provided, uses real boilerplate content from the
        official scaffold (root.hcl, common/ files) instead of hardcoded templates.
        ...
        Args:
            ...
            scaffold: Optional ScaffoldStructure with real boilerplate content.
        ...
        if plan.needs_root_config and not existing_project:
            files.extend(self._generate_root_config(plan, scaffold))

        # Generate common/ if needed
        if plan.needs_common and not existing_project:
            files.extend(self._generate_common(plan, scaffold))
        ...
        self.logger.info(
            "Assembled %d files for %d stacks (project_type=%s, scaffold=%s)",
            len(files),
            plan.stack_count,
            self.project_type,
            "loaded" if scaffold and scaffold.boilerplate else "default",
        )
        return files
    ...
    def _generate_root_config(
        self, plan: CompositionPlan, scaffold: Optional["ScaffoldStructure"] = None
    ) -> List[GeneratedFile]:
        """Generate root.hcl with remote_state and provider generate blocks.

        If a scaffold is provided and has a root.hcl boilerplate, uses the
        real scaffold content. Otherwise falls back to the hardcoded template.

        Args:
            plan: The composition plan for project metadata.
            scaffold: Optional scaffold with boilerplate content.

        Returns:
            List of GeneratedFile objects for root-level files.
        """
        files: List[GeneratedFile] = []

        # Use scaffold boilerplate for root.hcl if available
        if scaffold and "root.hcl" in scaffold.boilerplate:
            content = scaffold.boilerplate["root.hcl"]
            self.logger.debug("Using scaffold boilerplate for root.hcl")
        else:
            content = self._default_root_hcl()
            self.logger.debug("Using hardcoded default for root.hcl")

        files.append(GeneratedFile(path="root.hcl", content=content))

        # Also emit other root-level boilerplate files from scaffold
        if scaffold and scaffold.boilerplate:
            for filename, file_content in scaffold.boilerplate.items():
                # Skip common/ files (handled in _generate_common)
                # Skip root.hcl (already added above)
                if filename.startswith("common/") or filename == "root.hcl":
                    continue
                files.append(GeneratedFile(path=filename, content=file_content))
                self.logger.debug(f"Added scaffold boilerplate: {filename}")

        return files

    @staticmethod
    def _default_root_hcl() -> str:
        """Hardcoded default root.hcl when no scaffold is available."""
        return """\
...
"""
    ...
    def _generate_common(
        ...
        If a scaffold is provided and has common/ boilerplate files, uses the
        real scaffold content. Otherwise falls back to hardcoded templates.
        ...
        """
        # Check if scaffold has common/ files
        if scaffold and scaffold.boilerplate:
            common_files = {
                k: v
                for k, v in scaffold.boilerplate.items()
                if k.startswith("common/")
            }
            if common_files:
                self.logger.debug(
                    f"Using scaffold boilerplate for common/ ({len(common_files)} files)"
                )
                return [
        ...
        self.logger.debug("Using hardcoded defaults for common/")
        common_hcl = """\
