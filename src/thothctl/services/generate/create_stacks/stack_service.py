        ...
        try:
            ...
            for stack in config.get("stacks", []):
                ...
                for module_name in stack["modules"]:
                    ...
                    self.logger.info(
                        f"Generated configuration for {module_name} in {stack_name}"
                    )

        except Exception as e:
            self.logger.error(
                f"Failed to process YAML configuration: {e}", exc_info=True
            )
            raise ValueError(f"Failed to process YAML configuration: {e}")

    def generate_single_stack(
        self, stack_name: str, modules: List[str], output_dir: Path
    ) -> None:
    ...
    def create_example_config(self, output_file: Path) -> None:
        ...
        example_config = {
            ...
            "modules": [
                ...
                {
                    "name": "ec2-instance",
                    "variables": {"subnet_id": "vpc.private_subnets"},
                    "dependencies": ["vpc"],
                },
                {
                    "name": "rds",
        ...
        for file_name in stack_content:
            if file_name == "main.tf":
                with open(stack_dir / "main.tf", "w") as f:
                    f.write(
                        """# Main terraform configuration for this stack
...
"""
                ...
                with open(stack_dir / "graph.svg", "w") as f:
                    f.write(
                        """<!-- This file will be generated by running: terragrunt graph-dependencies | dot -Tsvg > graph.svg -->
...
"""
    ...
    def _create_module_config(
        self, module_dir: Path, module_name: str, module_config: dict
    ) -> None:
...
"""
        ...
        for var_name, var_value in variables.items():
            if isinstance(var_value, str) and "." in var_value:
                ...
                inputs_block += (
                    f"  {var_name} = dependency.{ref_module}.outputs.{output_name}\n"
                )
            else:
                # Handle direct value
...
"""
    ...
    def generate_stacks_from_config(self, config_file: Path, output_dir: Path) -> None:  # noqa: F811
        """
        Generate stacks from a YAML configuration file.
            ...
            for stack in config.get("stacks", []):
                ...
                for module_name in stack["modules"]:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Error generating config for {module_name}: {e}",
                            exc_info=True,
                        )
...
"""
    ...
    def create_example_config(self, output_file: Path) -> None:  # noqa: F811
        """
        Create an example YAML configuration file.
