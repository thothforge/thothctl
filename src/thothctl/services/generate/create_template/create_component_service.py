...
class ComponentCreator:
    ...
    def _create_hashable_key(folder_structure: Sequence[Dict]) -> tuple:
        ...
        return tuple(tuple(sorted(folder.items())) for folder in folder_structure)
    ...
    def get_folder_structure(
        folders_structure: Sequence[Dict], folder_name: str
    ) -> Optional[List]:
        """Get the content structure for a specific folder."""
        return next(
            (f["content"] for f in folders_structure if f["name"] == folder_name), None
        )
    ...
    def _build_folder_tree(self, folders: List[Dict]) -> Dict[str, FolderStructure]:
        ...
        logger.debug(
            f"Created folder map: {[f'{k}: parent={v.parent}' for k, v in folder_map.items()]}"
        )
        ...
        for folder_name, folder in folder_map.items():
            if folder.parent is None:
                ...
                if parent:
                    ...
                    logger.warning(
                        f"Parent folder {folder.parent} not found for {folder_name}"
                    )

        logger.debug(f"Root folders: {list(root_folders.keys())}")
        for root_name, root_folder in root_folders.items():
            logger.debug(
                f"Root {root_name} has children: {[child.name for child in root_folder.children]}"
            )
    ...
    def _create_file(self, base_path: Path, file_name: str, folder_name: str) -> None:
        ...
        try:
            content = FileTemplates.get_content(
                file_name, self._get_folder_name(folder_name)
            )
    ...
    def _create_component_structure(
        self, base_path: Path, folder: FolderStructure, is_root: bool = True
    ) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Creating structure for {'root' if is_root else 'child'} folder: {actual_folder_name}"
            )
            ...
            if folder.content:
                ...
                with ThreadPoolExecutor(
                    max_workers=min(len(folder.content), os.cpu_count() or 1)
                ) as executor:
                    list(
                        executor.map(
                            lambda f: self._create_file(
            ...
            logger.error(
                f"Error creating structure for folder {folder.name}", exc_info=True
            )
            raise
    ...
    def create(self) -> None:
        ...
        logger.info(
            f"Creating {self.config.component_type} "
        ...
        try:
            ...
            if self.config.component_type not in folder_names:
                raise ValueError(
                    f"Invalid component type: {self.config.component_type}"
                )
            ...
            if not root_folder:
                raise ValueError(
                    f"No structure defined for component type: {self.config.component_type}"
                )
    ...
    def _create_files_parallel(self, base_path: Path, file_names: List[str]) -> None:
        ...
        with ThreadPoolExecutor(
            max_workers=min(len(file_names), os.cpu_count() or 1)
        ) as executor:
