 ...
 #           'tfsec': Scanner('tfsec', TFSecScanner()),
    ...
    def _recursive_terraform_scan(
        ...
        try:
            ...
            if (Path(directory) / "main.tf").exists() or (Path(directory) / "tfplan.json").exists():
...
def recursive_scan(directory: str, tool: str, reports_dir: str, features_dir: str = "", options=None, tftool="tofu"):
        """
