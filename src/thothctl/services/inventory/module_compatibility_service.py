...
class ModuleCompatibilityService:
    ...
    def get_module_schema(
        ...
        try:
            if version == "latest":
                url = (
                    f"{self.registry_base_url}/v1/modules/{namespace}/{name}/{provider}"
                )
            else:
            ...
            logger.error(
                f"Failed to fetch module schema for {namespace}/{name}/{provider}@{version}: {str(e)}"
            )
            return None
        except Exception as e:
    ...
    def get_module_versions(
        ...
        try:
            ...
            logger.error(
                f"Failed to fetch module versions for {namespace}/{name}/{provider}: {str(e)}"
            )
            return []

    def _analyze_semantic_version_change(
        ...
        try:
            ...
            if is_major_change:
                ...
                module_warnings = self._get_known_module_warnings(
            ...
            logger.warning(
                f"Failed to parse version numbers {old_version} -> {new_version}: {e}"
            )
            return {
                "issues": [
                    CompatibilityIssue(
    ...
    def _get_known_module_warnings(
        self, old_version: str, new_version: str
    ...
    def compare_module_schemas(
        ...
        logger.info(
            f"Comparing module schemas: {module_name} {old_version} -> {new_version}"
        )

        # First, perform semantic version analysis
        semantic_analysis = self._analyze_semantic_version_change(
            old_version, new_version
        )
        ...
        if old_schema and new_schema:
            ...
            logger.warning(
                f"Unable to fetch detailed schema for {module_name}, using semantic version analysis"
            )
        ...
        if breaking_issues:
            ...
            upgrade_safe = True
            summary = (
                f"Found {len(warning_issues)} minor issue(s) that may need attention"
            )
        else:
    ...
    def check_module_compatibility(
        self, module_source: str, current_version: str, target_version: str = "latest"
    ) -> Optional[ModuleCompatibilityReport]:
        ...
        if target_version == "latest":
            ...
            if not versions:
                logger.warning(
                    f"No versions found for module {namespace}/{name}/{provider}"
                )
                return None
        ...
        if current_version == target_version:
            logger.debug(
                f"Module {namespace}/{name}/{provider} is already at target version {target_version}"
            )
            return None

        return self.compare_module_schemas(
            namespace, name, provider, current_version, target_version
        )

    def check_inventory_modules_compatibility(
                ...
                if component_type in [
                    ...
                    if isinstance(current_version, list) and current_version:
                        current_version = current_version[
                            0
                        ]  # Take the first version if it's a list

                    logger.info(
                        f"🔍 Found {component_type}: {source}, current: {current_version}, latest: {latest_version}"
                    )
                    ...
                    if (
                        ...
                        report = self.check_module_compatibility(
        ...
        inventory["module_compatibility"] = {
            ...
            "safe_upgrades": len(
                [r for r in compatibility_reports if r["upgrade_safe"]]
            ),
            "breaking_changes": len(
                [r for r in compatibility_reports if not r["upgrade_safe"]]
            ),
            "reports": compatibility_reports,
        }

        logger.info(
            f"Module compatibility analysis completed. Analyzed {len(compatibility_reports)} modules"
        )
