...
class InventoryService:
    ...
    def __init__(
        ...
        self.provider_version_service = (
            provider_version_service or ProviderVersionManager()
        )
    ...
    def _parse_terragrunt_file(
        self, file_path: Path, source_directory: Path = None
    ) -> List[Component]:
    ...
    def _walk_directory(
        self, directory: Path, complete: bool = False
    ...
    def _detect_project_type(self, source_path: Path, complete: bool = False) -> str:
        ...
        if version_files:
            ...
            for path in source_path.rglob("*.tf"):
                if path.parent != source_path and not self._should_exclude_path(
                    path, complete
                ):
                    has_subdir_tf = True
                    break
    ...
    async def create_inventory(
        ...
        for file_type, file_path in self._walk_directory(source_path, complete):
            ...
            if components:  # Only add if there are components
                ...
                if check_providers:
                    ...
                    providers = self._get_providers_for_stack(
                        abs_stack_path, provider_tool, terragrunt_args
                    )
                    ...
                    for provider in providers:
                        ...
                        # Track unique providers by name+version+source
                        provider_key = (
                            f"{provider.name}|{provider.version}|{provider.source}"
                        )
                    ...
                    if providers:
                        ...
                        logger.info(
                            f"Added {len(providers)} providers to stack {stack_name}"
                        )
        ...
        if check_provider_versions and check_providers and inventory_dict:
            ...
            if all_providers:
                ...
                updated_providers = (
                    await self.provider_version_service.check_provider_versions(
                        all_providers
                    )
                )
                ...
                for component_group in inventory_dict.get("components", []):
                    ...
                    if group_providers:
                        ...
                        updated_group_providers = updated_providers[
                            provider_index : provider_index + len(group_providers)
                        ]
                        component_group["providers"] = updated_group_providers
                        provider_index += len(group_providers)

                # Add provider version statistics
                outdated_providers = sum(
                    1 for p in updated_providers if p.get("status") == "outdated"
                )
                current_providers = sum(
                    1 for p in updated_providers if p.get("status") == "current"
                )
                unknown_providers = sum(
                    1 for p in updated_providers if p.get("status") == "unknown"
                )
        ...
        logger.info(
            f"Schema compatibility debug: check_schema_compatibility={check_schema_compatibility}"
        )
        logger.info(
            f"Schema compatibility debug: check_provider_versions={check_provider_versions}"
        )
        logger.info(
            f"Schema compatibility debug: inventory_dict exists={inventory_dict is not None}"
        )
        if inventory_dict:
            logger.info(
                f"Schema compatibility debug: inventory_dict has components={len(inventory_dict.get('components', []))}"
            )
        ...
        if check_schema_compatibility and check_provider_versions and inventory_dict:
            ...
            try:
                ...
                async def check_compatibility_async():
                    ...
                    logger.info(
                        f"🔍 Processing {len(inventory_dict.get('components', []))} component groups"
                    )

                    for component_group in inventory_dict.get("components", []):
                        providers = component_group.get("providers", [])
                        logger.info(
                            f"🔍 Component group has {len(providers)} providers"
                        )
                        ...
                        for provider in providers:
                            ...
                            logger.info(
                                f"🔍 Checking provider: {provider['name']} v{provider.get('version', 'latest')}, latest: {latest_version}"
                            )

                            if (
                                provider_key not in processed_providers
                                and latest_version
                            ):
                                processed_providers.add(provider_key)
                                logger.info(
                                    f"🔍 Processing compatibility for {provider['name']}"
                                )
                                ...
                                (
                                    namespace,
                                    provider_name,
                                ) = self._extract_provider_namespace(source_value)
                                logger.info(
                                    f"🔍 Extracted namespace: {namespace}, provider: {provider_name}"
                                )

                                # Get resources used by this provider (optional enhancement)
                                used_resources = (
                                    None  # Could be extracted from IaC files
                                )
                                ...
                                compatibility_report = await self.schema_compatibility_service.check_provider_compatibility(
                                    ...
                                    latest_version=provider.get(
                                        "latest_version",
                                        provider.get("version", "latest"),
                                    ),
                                ...
                                logger.info(
                                    f"🔍 Added compatibility report for {provider['name']}: {compatibility_report.compatibility_level.value}"
                                )
                            else:
                                if provider_key in processed_providers:
                                    logger.info(
                                        f"🔍 Skipping duplicate provider: {provider_key}"
                                    )
                                else:
                                    logger.info(
                                        f"🔍 Skipping provider without latest_version: {provider['name']}"
                                    )

                    logger.info(
                        f"🔍 Generated {len(compatibility_reports)} compatibility reports"
                    )
                ...
                if compatibility_reports:
                    inventory_dict["schema_compatibility"] = {
                        ...
                        "providers_with_breaking_changes": sum(
                            1 for r in compatibility_reports if r.breaking_changes
                        ),
                        "providers_with_warnings": sum(
                            1 for r in compatibility_reports if r.warnings
                        ),
                    }
                    ...
                    logger.info(
                        f"Schema compatibility analysis completed for {len(compatibility_reports)} providers"
                    )
                    logger.info(
                        f"Found {inventory_dict['schema_compatibility']['providers_with_breaking_changes']} providers with breaking changes"
                    )
                    logger.info(
                        f"Found {inventory_dict['schema_compatibility']['providers_with_warnings']} providers with warnings"
                    )
                else:
                ...
                logger.debug(
                    f"Schema compatibility error details: {str(e)}", exc_info=True
                )
                inventory_dict["schema_compatibility"] = {
                ...
                logger.info(
                    f"Provider version check completed: {outdated_providers} outdated, {current_providers} current, {unknown_providers} unknown"
                )
        ...
        if check_schema_compatibility and inventory_dict:
            ...
            try:
                inventory_dict = self.module_compatibility_service.check_inventory_modules_compatibility(
                    inventory_dict
                )

                module_stats = inventory_dict.get("module_compatibility", {})
                logger.info("Module compatibility analysis completed:")
                logger.info(
                    f"  - Total modules analyzed: {module_stats.get('total_modules_analyzed', 0)}"
                )
                logger.info(
                    f"  - Safe upgrades: {module_stats.get('safe_upgrades', 0)}"
                )
                logger.info(
                    f"  - Breaking changes: {module_stats.get('breaking_changes', 0)}"
                )

            except Exception as e:
                logger.error(f"Error during module compatibility checking: {str(e)}")
                logger.debug(
                    f"Module compatibility error details: {str(e)}", exc_info=True
                )
        ...
        if check_versions and inventory_dict:
            ...
            logger.info(
                f"Technical debt score: {tech_debt_metrics.get('debt_score', 0):.1f}%"
            )
        # Generate reports
        ...
        self.report_service.create_json_report(
            inventory_dict, reports_directory=str(reports_path)
        )
    ...
    def _get_providers_for_stack(
        self, stack_path: str, provider_tool: str = "tofu", terragrunt_args: str = ""
    ) -> List[Provider]:
        """
        ...
        if not abs_stack_path.exists() or not abs_stack_path.is_dir():
            logger.warning(
                f"Stack path does not exist or is not a directory: {stack_path}"
            )
            return providers
        ...
        try:
            ...
            logger.warning(
                f"Timeout checking for {tool_name} command. Skipping provider check."
            )
            return providers
        ...
        try:
            ...
            for provider in providers:
                ...
                if provider.module and (
                    provider.module.startswith("/") or "/" in provider.module
                ):
                    ...
                    module_name = (
                        provider.module.split(".")[-1]
                        if "." in provider.module
            ...
            for provider in providers:
                logger.info(
                    f"  Provider: {provider.name}, Version: {provider.version}, Source: {provider.source}, Module: {provider.module}"
                )
            return providers
        ...
        if not providers:
            ...
            if providers:
                logger.info(
                    f"Recovered {len(providers)} providers from HCL for {stack_path}"
                )
            ...
            for terraform_block in data.get("terraform", []):
                for rp in terraform_block.get("required_providers", []):
                    for name, config in rp.items():
                        source = (
                            config.get("source", f"hashicorp/{name}")
                            if isinstance(config, dict)
                            else f"hashicorp/{name}"
                        )
                        version = (
                            config.get("version", "~>")
                            if isinstance(config, dict)
    ...
    def _parse_providers_output(
        self, output: str, stack_path: str = ""
    ) -> List[Provider]:
        """
        ...
        for line in output.splitlines():
            ...
            if provider_match:
                ...
                version = (
                    provider_match.group(2) if provider_match.group(2) else "latest"
                )
                ...
                inline_component_match = re.search(
                    r'for\s+(resource|data)\s+"([^"]+)"\s+"([^"]+)"', line
                )
    ...
    def _detect_cdk_stacks(
        self, source_path: Path, language: str
        ...
        if language == "typescript":
            stack_pattern = re.compile(
                r"class\s+(\w+)\s+extends\s+\w*(?:Stack|NestedStack)"
            )
        else:
    ...
    async def _check_cdk_versions(
        self, inventory_dict: Dict[str, Any], registry: str
    ) -> Dict[str, Any]:
            ...
            for group in inventory_dict.get("components", []):
                for comp in group.get("components", []):
                    ...
                    if latest and latest != "Null" and local_ver != "Null":
                        comp["status"] = (
                            "Updated" if latest == local_ver else "Outdated"
                        )
                    else:
                        comp["status"] = "Unknown"
    ...
    async def _analyze_module(
        ...
        component_group = ComponentGroup(
            stack=f"./{module_name}", components=components, providers=providers
        )
        ...
        if check_versions and components:
            ...
            try:
                inventory_dict = await self.provider_version_service.check_versions(
                    inventory_dict, check_provider_versions=check_provider_versions
                )
            except Exception as e:
            ...
            try:
                ...
                for provider in providers:
                    ...
                    if provider_dict.get("source") and provider_dict.get("name"):
                        ...
                        logger.debug(
                            f"Provider dict for version check: {provider_dict}"
                        )
                ...
                if provider_dicts:
                    ...
                    logger.debug(
                        f"Updated providers from version service: {updated_providers}"
                    )
                ...
                logger.debug(
                    f"Provider version check error details: {e}", exc_info=True
                )
    ...
    def _calculate_technical_debt_metrics(
        ...
        metrics["modules_with_breaking_changes"] = module_compat.get(
            "breaking_changes", 0
        )

        schema_compat = inventory.get("schema_compatibility", {})
        metrics["providers_with_breaking_changes"] = schema_compat.get(
            "providers_with_breaking_changes", 0
        )
        ...
        module_debt = (
            (metrics["outdated_modules"] / total_modules) if total_modules > 0 else 0
        )
        provider_debt = (
            (metrics["outdated_providers"] / total_providers)
        ...
        if total_modules > 0 or total_providers > 0:
            metrics["debt_score"] = (
                module_debt * MODULE_WEIGHT + provider_debt * PROVIDER_WEIGHT
            ) * 100

            # Add penalty for breaking changes (+5 points each)
            breaking_weight = (
                metrics["modules_with_breaking_changes"]
                + metrics["providers_with_breaking_changes"]
            ) * 5
        ...
        if metrics["debt_score"] < 20:
            metrics["recommendations"].append(
                "Infrastructure is well-maintained. Continue regular updates."
            )
