...
class ProviderChangelogParser:
    ...
    def __init__(self):
        ...
        self.session.headers.update({"User-Agent": "ThothCTL/0.9.2 Changelog Parser"})

    def get_changelog_url(
        self, provider_name: str, namespace: str = "hashicorp"
    ) -> Optional[str]:
        """
        ...
        if not repo:
            ...
            logger.info(
                f"Provider {provider_name} not in known list, discovering from registry..."
            )
    ...
    def _discover_provider_repo(
        self, provider_name: str, namespace: str
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            if source:
                ...
                if "github.com" in source:
                    ...
                    if len(parts) >= 2:
                        ...
                        logger.info(
                            f"Discovered repository for {provider_name}: {repo}"
                        )
                        return repo
            ...
            logger.warning(
                f"Failed to parse registry response for {provider_name}: {e}"
            )
            return None
    ...
    def fetch_changelog(
        ...
        logger.warning(
            f"Failed to fetch changelog for {provider_name} from any location"
        )
        return None

    def get_upgrade_guide_url(
        self, provider_name: str, major_version: str, namespace: str = "hashicorp"
    ) -> Optional[str]:
        ...
        for line in lines:
            ...
            if version_match:
                ...
                if current_version and in_range and current_section:
                    entries.extend(
                        self._parse_version_section(current_version, current_section)
                    )
    ...
    def _parse_version_section(
        self, version: str, lines: List[str]
    ) -> List[ChangelogEntry]:
            ...
            if line.startswith("*") or line.startswith("-"):
                ...
                if not current_type:
                    if any(
                        ...
                        for keyword in ["breaking", "removed", "incompatible"]
                    ):
                        current_type = "breaking"
                    elif any(
                        keyword in description.lower()
                        for keyword in ["deprecat", "will be removed"]
                    ):
                        current_type = "deprecated"
                    else:
                        current_type = "note"
    ...
    def get_breaking_changes_summary(
        ...
        try:
            ...
            upgrade_guide_url = self.get_upgrade_guide_url(
                provider_name, str(major_version), namespace
            )
        except Exception:
