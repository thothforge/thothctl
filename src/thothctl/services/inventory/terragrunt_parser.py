...
class TerragruntParser:
    ...
    def parse_terragrunt_file(
        ...
        try:
            ...
            try:
                ...
                components = self._extract_components_from_hcl(
                    data, file_path, source_directory
                )
                if components:
                    logger.debug(
                        f"Successfully parsed {file_path} with hcl2, found {len(components)} components"
                    )
                    return components
            ...
            components = self._extract_components_with_regex(
                content, file_path, source_directory
            )
            logger.debug(
                f"Regex parsing of {file_path} found {len(components)} components"
            )
            return components
    ...
    def _extract_components_from_hcl(
        self, data: dict, file_path: Path, source_directory: Path = None
    ) -> List[Component]:
        ...
        if "terraform" in data:
            ...
            if isinstance(terraform_blocks, list):
                for block in terraform_blocks:
                    component = self._process_terraform_block(
                        block, file_path, source_directory
                    )
                    if component:
                        components.append(component)
            elif isinstance(terraform_blocks, dict):
                component = self._process_terraform_block(
                    terraform_blocks, file_path, source_directory
                )
                if component:
                    components.append(component)
    ...
    def _process_terraform_block(
        self, terraform_block: dict, file_path: Path, source_directory: Path = None
    ) -> Component:
    ...
    def _extract_components_with_regex(
        self, content: str, file_path: Path, source_directory: Path = None
    ) -> List[Component]:
        """Extract components using regex parsing as fallback."""
        components = []

        # Regex to find terraform blocks with source
        terraform_block_pattern = (
