...
class SchemaCompatibilityService:
    ...
    async def check_provider_compatibility(
        ...
        try:
            logger.info(
                f"Checking compatibility for {provider_name}: {current_version} -> {latest_version}"
            )

            # Skip if versions are the same
            if current_version == latest_version:
                logger.debug(
                    f"Versions are identical for {provider_name}, creating same-version report"
                )
            ...
            if current_version == "latest":
                logger.info(
                    f"Current version is 'latest' for {provider_name}, assuming compatible"
                )
                return self._create_latest_version_report(provider_name, latest_version)

            # Get schemas for both versions
            logger.debug(
                f"Fetching schema for {namespace}/{provider_name} {current_version}"
            )
            current_schema = await self._get_provider_schema(
                provider_name, current_version, namespace
            )

            logger.debug(
                f"Fetching schema for {namespace}/{provider_name} {latest_version}"
            )
            latest_schema = await self._get_provider_schema(
                provider_name, latest_version, namespace
            )
            ...
            if not current_schema:
                logger.warning(
                    f"Could not retrieve current schema for {provider_name} {current_version}"
                )
            ...
            if not latest_schema:
                logger.warning(
                    f"Could not retrieve latest schema for {provider_name} {latest_version}"
                )
                return self._create_partial_report(
                    provider_name,
                    current_version,
            ...
            logger.info(
                f"Compatibility analysis completed for {provider_name}: {report.compatibility_level.value}"
            )
            return report
            ...
            logger.error(
                f"Network error checking compatibility for {provider_name}: {str(e)}"
            )
            return self._create_error_report(
                provider_name,
                current_version,
            ...
            logger.error(
                f"Unexpected error checking compatibility for {provider_name}: {str(e)}"
            )
            logger.debug(
                f"Full error details for {provider_name}: {str(e)}", exc_info=True
            )
            return self._create_error_report(
                provider_name,
                current_version,
    ...
    def _create_latest_version_report(
        self, provider_name: str, version: str
    ) -> CompatibilityReport:
        ...
        return CompatibilityReport(
            ...
            recommendations=[
                "Consider pinning to specific version for reproducible builds"
            ],
        )
    ...
    async def _get_provider_schema(
        self, provider_name: str, version: str, namespace: str = "hashicorp"
    ) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(
                f"🔍 Fetching schema for {namespace}/{provider_name} v{version}"
            )
            ...
            if response.status_code != 200:
                logger.warning(
                    f"Failed to fetch provider version info: {response.status_code}"
                )
                return await self._get_fallback_schema(
                    provider_name, version, namespace
                )
            ...
            if "platforms" in version_data:
                for platform_info in version_data["platforms"]:
                    if (
                        ...
                        and platform_info.get("arch") == "amd64"
                    ):
            ...
            if not download_url:
                logger.debug(
                    f"No download URL found for {provider_name} {version}, using docs API"
                )
            ...
            docs_schema = await self._get_schema_from_docs(
                namespace, provider_name, version
            )

            if docs_schema:
                self.cache[cache_key] = docs_schema
                logger.info(
                    f"✅ Retrieved schema for {provider_name} {version} from documentation"
                )
                return docs_schema

            # If docs approach fails, try the provider releases API
            releases_schema = await self._get_schema_from_releases(
                namespace, provider_name, version
            )
            ...
            if releases_schema:
                ...
                logger.info(
                    f"✅ Retrieved schema for {provider_name} {version} from releases"
                )
                return releases_schema

            # Final fallback
            logger.warning(
                f"Could not retrieve detailed schema for {provider_name} {version}, using enhanced fallback"
            )
            fallback_schema = await self._get_enhanced_fallback_schema(
                provider_name, version, namespace
            )
            ...
            logger.error(
                f"Error retrieving schema for {provider_name} {version}: {str(e)}"
            )
                            ...
                            resources.append(
                                {
                                    ...
                                    "path": item.get("path", ""),
                                    "attributes": self._extract_attributes_from_doc(
                                        item
                                    ),
                                }
                            )
    ...
    async def _get_schema_from_docs(
        self, namespace: str, provider_name: str, version: str
    ) -> Optional[Dict]:
    ...
    async def _get_schema_from_releases(
        self, namespace: str, provider_name: str, version: str
    ) -> Optional[Dict]:
    ...
    async def _get_enhanced_fallback_schema(
        self, provider_name: str, version: str, namespace: str
    ) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    "estimated_resources": self._estimate_resources_by_version(
    ...
    async def _get_fallback_schema(
        self, provider_name: str, version: str, namespace: str
    ) -> Optional[Dict]:
    ...
    def _analyze_schema_differences(
        ...
        try:
            ...
            logger.info(
                f"🔍 Analyzing schema differences for {provider_name}: {current_version} -> {latest_version}"
            )
            ...
            current_resource_count = len(current_resources) or current_est.get(
                "resources", 0
            )
            latest_resource_count = len(latest_resources) or latest_est.get(
                "resources", 0
            )

            current_ds_count = len(current_data_sources) or current_est.get(
                "data_sources", 0
            )
            latest_ds_count = len(latest_data_sources) or latest_est.get(
                "data_sources", 0
            )

            logger.info(
                f"🔍 Resource comparison: {current_resource_count} -> {latest_resource_count}"
            )
            logger.info(
                f"🔍 Data source comparison: {current_ds_count} -> {latest_ds_count}"
            )

            # Analyze version differences
            version_diff = self._analyze_version_difference(
            ...
            if version_diff["major_change"]:
                ...
                changes.extend(
                    self._generate_major_version_changes(
                        provider_name,
                        current_version,
                        latest_version,
                        current_resource_count,
                        latest_resource_count,
                    )
                )
            ...
            if version_diff["minor_change"] or version_diff["significant"]:
                ...
                changes.extend(
                    self._generate_minor_version_changes(
                        provider_name,
                        current_version,
                        latest_version,
                        current_resource_count,
            ...
            if current_resources and latest_resources:
                changes.extend(
                    self._compare_resources(
                        current_resources, latest_resources, used_resources
                    )
                )
            ...
            if current_data_sources and latest_data_sources:
                changes.extend(
                    self._compare_data_sources(
                        current_data_sources, latest_data_sources, used_resources
                    )
                )
            ...
            if provider_name == "aws":
                changes.extend(
                    self._analyze_aws_specific_changes(current_version, latest_version)
                )
            elif provider_name == "google":
                changes.extend(
                    self._analyze_google_specific_changes(
                        current_version, latest_version
                    )
                )
            elif provider_name == "azurerm":
                changes.extend(
                    self._analyze_azure_specific_changes(
                        current_version, latest_version
                    )
                )
    ...
    def _analyze_version_difference(self, current: str, latest: str) -> Dict:
        ...
        try:
            ...
            minor_change = (
                latest_parts[1] > current_parts[1] if len(latest_parts) > 1 else False
            )
            patch_change = (
                latest_parts[2] > current_parts[2] if len(latest_parts) > 2 else False
            )

            version_gap = (
                (latest_parts[0] - current_parts[0]) * 100
            ...
            return {
                ...
                "significant": version_gap
                > 50,  # Arbitrary threshold for significant changes
            }
    ...
    def _generate_major_version_changes(
        ...
        if (
            provider_name == "aws"
            and current_version.startswith("4.")
            and latest_version.startswith("6.")
        ):
    ...
    def _analyze_aws_specific_changes(
    ...
    def _analyze_google_specific_changes(
        self, current_version: str, latest_version: str
    ) -> List[SchemaChange]:
    ...
    def _analyze_azure_specific_changes(
        self, current_version: str, latest_version: str
    ) -> List[SchemaChange]:
    ...
    def _compare_resources(
        ...
        latest_resources: List[Dict],
        used_resources: Optional[List[str]],
    ) -> List[SchemaChange]:
    ...
    def _compare_data_sources(
        self,
        current_data_sources: List[Dict],
        latest_data_sources: List[Dict],
        used_resources: Optional[List[str]],
    ) -> List[SchemaChange]:
        ...
        current_names = {
            ds.get("name", ds.get("path", "")) for ds in current_data_sources
        }
        latest_names = {
            ds.get("name", ds.get("path", "")) for ds in latest_data_sources
        }
    ...
    def _create_compatibility_report(
        ...
        breaking_changes = [
            c
            for c in changes
            if c.severity in ["error", "high"]
            or c.type in ["breaking_change", "major_version_upgrade"]
        ]
        ...
        deprecations = [
            c
            for c in changes
            if "deprecated" in c.description.lower() or c.type == "deprecation"
        ]
    ...
    def _generate_summary(
        ...
        if breaking_changes:
            return (
                f"⚠️ Breaking changes detected when upgrading {provider_name} "
            ...
            return (
                f"⚡ Minor issues detected when upgrading {provider_name} "
                f"from {current_version} to {latest_version}. "
                f"{len(warnings)} warnings, {len(new_features)} new features."
            )
        else:
            return (
                f"✅ {provider_name} upgrade from {current_version} to {latest_version} "
    ...
    def _create_same_version_report(
        self, provider_name: str, version: str
    ) -> CompatibilityReport:
    ...
    def _create_unknown_report(
        ...
        current_version: str,
        latest_version: str,
        reason: str = "Schema information unavailable",
    ) -> CompatibilityReport:
    ...
    def generate_compatibility_html_report(
