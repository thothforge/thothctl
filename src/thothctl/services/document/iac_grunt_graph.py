...
class CommandExecutor(Protocol):
    ...
    def execute(
        self, command: List[str], cwd: Path, input_data: str = None
    ) -> Tuple[str, str, int]:
        """
...
class DependencyGraphGenerator:
    ...
    def _generate_mermaid(self, directory: Path) -> GraphResult:
        ...
        try:
            ...
            self.logger.debug(
                "Successfully created mermaid diagram at: %s", mermaid_path
            )
            return GraphResult(success=True, content=mermaid_content, path=mermaid_path)
    ...
    def _prepare_paths(self, config: GraphConfig) -> Optional[Path]:
        ...
        try:
            ...
            print(
                f"{Fore.GREEN}Creating dependencies graph for {dir_path.name}{Fore.RESET}"
            )
    ...
    def _generate_graph(self, directory: Path) -> Optional[str]:
        ...
        enhanced_content = self._enhance_graph_with_dependencies(
            processed_content, directory
        )
                    ...
                    for dep_block in parsed["dependency"]:
                        for dep_name, dep_config in dep_block.items():
                            config_path = (
                                ...
                                if isinstance(dep_config.get("config_path"), list)
                                else dep_config.get("config_path", "")
                            )
                            mock_outputs_list = dep_config.get("mock_outputs", [])
                            mock_outputs = (
                                mock_outputs_list[0] if mock_outputs_list else {}
                            )
    ...
    def _enhance_graph_with_dependencies(
        self, dot_content: str, directory: Path
    ) -> str:
                        ...
                        if hcl_file.exists():
                            ...
                            if deps:
                                ...
                                for dep_name, dep_info in deps.items():
                                    mock_keys = (
                                        list(dep_info["mock_outputs"].keys())
                                        if dep_info["mock_outputs"]
                                        else []
                                    )
                                    if mock_keys:
                                        dep_labels.append(
                                            f"{dep_name}: {', '.join(mock_keys[:3])}"
                                        )
    ...
    def _create_svg(self, directory: Path, graph_content: str) -> GraphResult:
        ...
        try:
            stdout, stderr, return_code = self.executor.execute(
                ["dot", "-Tsvg"], directory, input_data=graph_content
            )
            ...
            return GraphResult(success=True, content=stdout, path=svg_path)
...
def setup_graph_generator() -> DependencyGraphGenerator:
    ...
    return DependencyGraphGenerator(executor=SubprocessExecutor(), logger=logger)
...
def find_terragrunt_files(
    start_path: Path, exclude_patterns: List[str] = None
...
def process_single_directory(
    ...
    try:
        ...
        return RecursiveGraphResult(directory=directory, result=None, error=str(e))
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_dir):
            ...
            try:
                ...
                results.append(
                    RecursiveGraphResult(directory=directory, result=None, error=str(e))
                )
