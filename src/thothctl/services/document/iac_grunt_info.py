...
class TerragruntInfoGenerator:
    ...
    def _get_module_description(self, config: Dict[str, Any]) -> str:
        ...
        try:
            ...
            return locals_block.get(
                "description", "Terragrunt configuration for infrastructure deployment"
            )
        except Exception:
...
"""
            ...
            self.logger.error(
                f"Error generating info file for {terragrunt_path}: {str(e)}"
            )
            return False
