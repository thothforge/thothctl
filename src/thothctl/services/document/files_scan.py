...
class FileScanner:
    ...
    def __init__(self, exclude_patterns: List[str] = None, max_workers: int = 4):
        """
    ...
    def process_files(
        self, files: Set[Path], processor: Callable[[Path], bool]
    ) -> dict:
        """
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_file = {executor.submit(processor, file): file for file in files}
    ...
    def scan_and_process(
        ...
        pattern: str = "*",
        recursive: bool = True,
        processor: Optional[Callable[[Path], bool]] = None,
    ) -> dict:
        """
