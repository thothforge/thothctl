    ...
    def get_offline_estimate(
        ...
        if resource_type == "aws_msk_cluster":
            ...
            if isinstance(broker_node_group_info, list):
                broker_info = (
                    broker_node_group_info[0] if broker_node_group_info else {}
                )
            else:
            ...
            total_storage_cost = (
                storage_per_broker * number_of_broker_nodes * self.storage_cost
            )
        ...
        return self._create_resource_cost(
            resource_change, component_type, region, hourly_cost, "medium"
        )
