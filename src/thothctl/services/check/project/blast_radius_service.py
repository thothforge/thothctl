...
class BlastRadiusService:
    ...
    def assess_blast_radius(
        self, directory: str, recursive: bool = False, plan_file: str = None
        ...
        affected_components = self._calculate_blast_radius(
            dependencies, planned_changes
        )
        ...
        recommendations = self._generate_recommendations(
            risk_level, affected_components
        )
        ...
        mitigation_steps = self._create_mitigation_steps(
            risk_level, affected_components
        )
    ...
    def _get_dependency_graph(self, directory: str, recursive: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "nodes": nodes,
                "edges": edges,
                "risks": risk_service.calculate_risk_for_components(
    ...
    def _get_planned_changes(
        self, directory: str, plan_file: str = None
    ) -> Dict[str, Any]:
    ...
    def _calculate_blast_radius(
        self, dependencies: Dict[str, Any], changes: Dict[str, Any]
    ) -> List[BlastRadiusComponent]:
        ...
        blast_radius_components = self._propagate_changes(
            changed_components, dependencies.get("edges", [])
        )
        ...
        for component in blast_radius_components:
            ...
            deps, dependents = self._get_component_relationships(
                component, dependencies.get("edges", [])
            )
            ...
            affected.append(
                BlastRadiusComponent(
                    ...
                    criticality=self._assess_component_criticality(
    ...
    def _assess_overall_risk(
        self, components: List[BlastRadiusComponent]
    ) -> ChangeRisk:
    ...
    def _determine_change_type(
        self, risk_level: ChangeRisk, components: List[BlastRadiusComponent]
    ) -> ChangeType:
    ...
    def _generate_recommendations(
        ...
        if any(comp.change_type == "replace" for comp in components):
            recommendations.append(
                "🔄 Resource replacement detected - ensure data backup"
            )
    ...
    def _create_mitigation_steps(
        ...
        if critical_components:
            steps.append(
                f"7. Extra validation for critical components: {', '.join(c.name for c in critical_components)}"
            )
    ...
    def _propagate_changes(
        self, changed_components: Set[str], edges: List[Tuple[str, str]]
    ) -> Set[str]:
    ...
    def _get_component_relationships(
        self, component: str, edges: List[Tuple[str, str]]
    ...
    def _get_change_type_for_component(
        self, component: str, changes: Dict[str, Any]
    ) -> str:
