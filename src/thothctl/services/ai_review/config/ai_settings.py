...
class AISettings:
    ...
    providers: Dict[str, ProviderConfig] = field(
        default_factory=lambda: {
            ...
            "bedrock": ProviderConfig(model="anthropic.claude-sonnet-4-6"),
            "bedrock_agent": ProviderConfig(model="anthropic.claude-sonnet-4-6"),
    ...
    def load(cls, config_path: Optional[str] = None) -> "AISettings":
        ...
        path = (
            Path(config_path)
            if config_path
            else Path(DEFAULT_CONFIG_DIR) / DEFAULT_CONFIG_FILE
        )
        if path.exists():
            try:
                with open(path) as f:
                ...
                settings.default_provider = ai_data.get(
        ...
        settings.default_provider = os.environ.get(
            "THOTH_AI_PROVIDER", settings.default_provider
        )
        ...
        if os.environ.get("AWS_DEFAULT_REGION"):
            ...
            settings.providers["bedrock_agent"].region = os.environ[
                "AWS_DEFAULT_REGION"
            ]
    ...
    def get_provider_config(
        ...
        if name not in self.providers:
            raise ValueError(
                f"Unknown AI provider: {name}. Available: {list(self.providers.keys())}"
            )
