...
class CostTracker:
    ...
    def get_monthly_spend(self) -> float:
        ...
        return sum(
            r.cost for r in self._records if r.timestamp.startswith(month_prefix)
        )
    ...
    @staticmethod
    def _calculate_cost(
        model: str, input_tokens: int, output_tokens: int, provider: str = ""
    ) -> float:
        if provider == "ollama":
            return 0.0
