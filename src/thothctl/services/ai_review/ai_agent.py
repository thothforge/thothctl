...
class AIReviewAgent:
    ...
    def analyze_directory(self, directory: str) -> Dict[str, Any]:
        ...
        with span("ai_agent.analyze_directory", {"directory": directory}) as s:
            ...
            self._emit_status(
                "📦 Collecting project context (inventory, scan reports, plans)..."
            )
            ...
            if ctx.scan_results.get("total_findings", 0) > 0:
                self._emit_status(
                    f"⚠️  Found {ctx.scan_results['total_findings']} scan findings, assessing risk..."
                )
            ...
            if self._provider.name == "ollama":
                ...
                if "qwen" in self._provider.model.lower():
                    fallback = (
                        (lambda: self._basic_analysis(ctx.scan_results, basic_risk))
                        if basic_risk
                    ...
                    base_result = (
                        self._basic_analysis(ctx.scan_results, basic_risk)
                        if basic_risk
                        else self._empty_result()
                    )
                    try:
                        ai_response = self._provider.analyze(SYSTEM_COMPACT, formatted)
                        if isinstance(ai_response, dict) and ai_response.get(
                            "recommendations"
                        ):
                            base_result["recommendations"] = ai_response[
                                "recommendations"
                            ]
                            base_result["architecture_assessment"] = ai_response.get(
                                "architecture_assessment", ""
                            )
                        elif isinstance(ai_response, dict) and ai_response.get(
                            "_raw_text"
                        ):
                            base_result["recommendations"] = [
                                ai_response["_raw_text"][:2000]
                            ]
                ...
                result = self._call_ai(
                    system_prompt, formatted, "analyze_directory", fallback=fallback
                )
    ...
    def review_code(self, directory: str) -> Dict[str, Any]:
        ...
        if not code_files:
            return {
                ...
                "issues": [],
                "overall_assessment": "No IaC files found.",
            }
    ...
    def generate_fixes(
        self, directory: str, scan_results: Dict = None, severity_filter: str = None
    ) -> Dict[str, Any]:
        """Generate actionable code fixes for scan findings."""
        with span(
            "ai_agent.generate_fixes",
            {"directory": directory, "severity_filter": severity_filter or "all"},
        ) as s:
            if not scan_results:
            ...
            content = self._format_fix_request(
                scan_results, code_files, severity_filter
            )
            ...
            result = self._call_ai(
                ...
                content,
                "generate_fixes",
                fallback=lambda: self._pattern_fixes(
                    scan_results, code_files, severity_filter
                ),
            )
    ...
    def _format_fix_request(
        self, scan_results: Dict, code_files: Dict, severity_filter: str = None
    ) -> str:
        ...
        for tool, data in scan_results.get("tools", {}).items():
            for f in data.get("findings", []):
                ...
                if severity_order.get(sev, 2) <= min_sev:
                    ...
                    if (
                        ".terraform/modules/" in file_path
                        or ".terraform\\modules\\" in file_path
                    ):
    ...
    def _load_org_policy_context(self) -> str:
        ...
        for rego in (Path(org_path) / "shared" / "policy").glob("*.rego"):
            ...
            for line in text.splitlines():
                if line.startswith(
                    ("required_tags", "naming_pattern", "deny[", "warn[")
                ):
                    parts.append(line.strip())
    ...
    def _fixes_from_analysis(self, analysis: Dict, directory: str) -> Dict:
        ...
        for i, rec in enumerate(analysis.get("recommendations", [])):
            if isinstance(rec, str) and any(
                kw in rec.lower() for kw in ["add", "enable", "configure", "set", "use"]
            ):
                fixes.append(
                    {
                        "fix_id": f"rec_{i:03d}",
    ...
    @staticmethod
    def _pattern_fixes(
        scan_results: Dict, code_files: Dict, severity_filter: str = None
    ) -> Dict:
        ...
        for tool, data in scan_results.get("tools", {}).items():
            for f in data.get("findings", []):
                ...
                if fix:
                    ...
                    skipped.append(
                        {"finding_id": check_id, "reason": "No pattern available"}
                    )
    ...
    def _call_ai(
        self, system_prompt: str, user_content: str, operation: str, fallback=None
    ) -> Dict[str, Any]:
