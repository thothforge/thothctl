...
def main():
    ...
    if system == "Windows":
        ...
        if shell not in ["bash", "zsh", "fish"]:
            print(
                f"Warning: Shell '{shell}' may not be fully supported. Defaulting to bash completion."
            )
            shell = "bash"
    ...
    print(f"\n    {completion_cmd}\n")
    print("For example, you can add it with:")
    ...
    answer = input(
        f"Would you like to add this line to {config_file} automatically? (y/n): "
    )
    if answer.lower() in ("y", "yes"):
        try:
            ...
            print(
                f"Please restart your shell or run 'source {config_file}' to activate it."
            )
