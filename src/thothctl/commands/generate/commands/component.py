...
from ....services.generate.create_template.create_component_service import (
    create_component,
)
...
class GenComponentCommand(ClickCommand):
    ...
    def _execute(self, **kwargs) -> Any:
        ...
        ctx.obj.get("CODE_DIRECTORY")

        try:
            self.ui.print_info(
                f"Creating component {kwargs.get('component_name')} in {kwargs.get('component_path')}"
            )
            create_component(
...
cli = GenComponentCommand.as_click_command(
    ...
    click.option("-cn", "--component-name", help="Component name for template"),
    click.option(
        "-cph",
        "--component-path",
