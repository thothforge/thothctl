...
class DocumentIaCCommand(ClickCommand):
    ...
    def __init__(self):
        super().__init__()
        self.ui = CliUI()
        self.supported_iac_types = [
            "terraform",
            "terragrunt",
            "terraform-terragrunt",
        ]  # 'cloudformation', 'ansible', 'kubernetes']
    ...
    def get_completions(
        ...
        if "--type" in args:
            return [
                CompletionItem(
                    iac_type, help=f"Document {iac_type} infrastructure code"
                )
    ...
    def validate(self, **kwargs) -> bool:
        ...
        if kwargs["framework"] not in self.supported_iac_types:
            self.logger.error(
                f"Unsupported IaC framework. Must be one of: {', '.join(self.supported_iac_types)}"
            )
            return False
    ...
    def _execute(self, **kwargs) -> Any:
        ...
        try:
            ...
            self._generate_documentation(
                ...
                recursive=recursive,
                exclude=kwargs.get(
                    "exclude", [".terraform", ".git", ".terragrunt-cache"]
                ),
                framework=framework,
...
cli = DocumentIaCCommand.as_click_command(
    ...
    click.option(
        ...
        "--framework",
        type=click.Choice(
            ["terraform", "terragrunt", "terraform-terragrunt"], case_sensitive=False
        ),
        required=True,
