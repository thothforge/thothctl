...
class ProjectBootstrapCommand(ClickCommand):
        ...
        **kwargs,
        ...
        try:
            ...
            if result["success"]:
                ...
                self.ui.print_error(
                    f"❌ Bootstrap failed: {result.get('error', 'Unknown error')}"
                )
    ...
    def _display_dry_run_results(self, result: dict) -> None:
        ...
        self.ui.print_info("📋 Project Analysis:")
        self.ui.print_info(
            f"  📁 Project Type: {project_info.get('detected_type', 'unknown')}"
        )
        self.ui.print_info(
            f"  📄 Has .thothcf.toml: {'Yes' if project_info.get('has_config') else 'No'}"
        )
    ...
    def _display_bootstrap_results(self, result: dict) -> None:
        ...
        if changes.get("skipped"):
            self.ui.print_warning(
                f"  ⏭️  Skipped {len(changes['skipped'])} existing files"
            )
