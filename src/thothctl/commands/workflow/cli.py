...
class WorkflowCLI(click.Group):
    """Custom Click Group class for workflow commands."""
    ...
    def _load_commands(self):
        ...
        try:
            ...
            logger.error(f"Error loading workflow subcommands: {e}")
    ...
    def get_command(self, ctx, cmd_name):
        try:
            ...
            module_name = f"thothctl.commands.workflow.commands.{cmd_name}"
            ...
            logger.error(f"Error loading workflow subcommand {cmd_name}: {str(e)}")
            return None


@click.group(cls=WorkflowCLI)
@click.pass_context
def cli(ctx):
    """Execute DevSecOps workflow phases."""
    pass
