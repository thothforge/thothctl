    ...
    def _execute(
        ...
        **kwargs,
        ...
        applicable = [
            f
            for f in fixes
            if f.get("replacement") and f.get("file") and f.get("fix_type") != "manual"
        ]
        manual = [
            f
            for f in fixes
            if f.get("fix_type") == "manual" or not f.get("replacement")
        ]
        ...
        for fix in applicable:
            ...
            if dry_run:
                self.ui.console.print(
                    f"[cyan]{fix_id}[/cyan] → {fix['file']}: {fix.get('description', '')}"
                )
            ...
            if fix["fix_type"] == "add_resource":
                ...
                self.ui.print_warning(
                    f"  {fix_id}: Cannot auto-apply — original not found in {fix['file']}"
                )
                failed += 1
        ...
        if manual:
            ...
            for fix in manual:
                self.ui.console.print(
                    f"  - {fix.get('fix_id', '?')}: {fix.get('description', '')}"
                )
...
cli = ApplyFixCommand.as_click_command(name="apply-fix")(
    ...
    click.option(
        ...
        "--directory",
        type=click.Path(exists=True),
        help="Base directory for file paths",
    ),
)
