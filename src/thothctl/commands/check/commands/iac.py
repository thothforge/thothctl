...
class CheckIaCCommand(ClickCommand):
    ...
    def validate(self, **kwargs) -> bool:
        ...
        if kwargs["check_type"] not in self.supported_check_types:
            self.logger.error(
                f"Unsupported Check type. Must be one of: {', '.join(self.supported_check_types)}"
            )
            return False
    ...
    def post_execute(self, **kwargs) -> None:
        ...
        # Drift detection results
            ...
            from ....services.check.project.drift.drift_report import (
    ...
    def _build_cost_markdown(self, cost_results: list) -> str:
        ...
        for r in cost_results:
            a = r["analysis"]
            lines.append(
                f"| {r['stack']} | ${a.total_monthly_cost:.2f} | ${a.total_annual_cost:.2f} |"
            )

        lines.append(
            f"| **TOTAL** | **${total_monthly:.2f}** | **${total_annual:.2f}** |"
        )
    ...
    def _build_blast_radius_markdown(self, assessment) -> str:
        ...
        if assessment.affected_components:
            ...
            for comp in assessment.affected_components:
                lines.append(
                    f"| {comp.name} | {comp.change_type} | {comp.risk_score:.2f} | {comp.criticality} |"
                )
    ...
    def _validate_tfplan(
        ...
        self.logger.info(
            f"Validating {tftool} plan in {directory} (recursive: {recursive})"
        )
        ...
        if not tfplan_files:
            self.console.print(
                f"[yellow]No {tftool} JSON plan files found in the specified directory[/yellow]"
            )
            return False
    ...
    def _find_tfplan_files(
        self, directory: str, recursive: bool, prefer_json: bool = True
    ) -> List[str]:
    ...
    def _process_tfplan_file(
        self, tfplan_file: str, dependencies: bool, tftool: str
    ) -> Dict:
        ...
        result = {
            "file": tfplan_file,
            "resources": [],
            "changes": {"create": [], "update": [], "delete": [], "no_op": []},
            "issues": [],
            "dependencies": [],
        }
    ...
    def _extract_dependencies_from_json(self, plan_data: Dict, result: Dict) -> None:
        ...
        if "configuration" in plan_data and "root_module" in plan_data["configuration"]:
            deps = []
            for resource in plan_data["configuration"]["root_module"].get(
                "resources", []
            ):
    ...
    def _generate_summary(self, results: List[Dict]) -> Dict:
        ...
        summary = {
            ...
            "total_resources": sum(
                len(result.get("resources", [])) for result in results
            ),
    ...
    def _display_rich_output(self, summary: Dict, results: List[Dict], tftool: str):
        ...
        self.console.print(
            Panel(
                f"[bold blue]{tftool.capitalize()} Plan Analysis Summary[/bold blue]",
                box=box.ROUNDED,
            )
        )
        ...
        changes_table.add_row(
            "Create", f"[green]{summary['changes']['create']}[/green]"
        )
        changes_table.add_row(
            "Update", f"[yellow]{summary['changes']['update']}[/yellow]"
        )
        changes_table.add_row("Delete", f"[red]{summary['changes']['delete']}[/red]")
        changes_table.add_row(
            "No Change", f"[blue]{summary['changes']['no_op']}[/blue]"
        )
        changes_table.add_row(
            "Total",
        ...
        if (
            summary["changes"]["create"] > 0
            or summary["changes"]["update"] > 0
            or summary["changes"]["delete"] > 0
        ):
            ...
            if summary["changes"]["create"] > 0:
                ...
                for result in results:
                    for resource in result["changes"]["create"]:
                        create_node.add(
                            f"[green]{resource['address']} ({resource['provider']})[/green]"
                        )
            ...
            if summary["changes"]["update"] > 0:
                ...
                for result in results:
                    for resource in result["changes"]["update"]:
                        update_node.add(
                            f"[yellow]{resource['address']} ({resource['provider']})[/yellow]"
                        )
            ...
            if summary["changes"]["delete"] > 0:
                ...
                for result in results:
                    for resource in result["changes"]["delete"]:
                        delete_node.add(
                            f"[red]{resource['address']} ({resource['provider']})[/red]"
                        )
    ...
    def _generate_markdown(
        self, outmd: str, summary: Dict, results: List[Dict], tftool: str
    ):
    ...
    def _visualize_dependencies(
        self, directory: str, dagtool: str = "terragrunt", output_format: str = "tree"
    ) -> bool:
    ...
    def _render_boxart(self, dot_graph: str, directory: str) -> bool:
        ...
        try:
            ...
            self.console.print(
                Panel("[bold blue]🔄 Dependency Topology[/bold blue]", box=box.ROUNDED)
            )
        ...
        self.console.print(
            Panel(
                "[bold blue]🔄 Dependency Topology (boxart)[/bold blue]",
                box=box.ROUNDED,
            )
        )
        self.console.print(
            "[dim]Install graph-easy for better rendering: sudo apt install libgraph-easy-perl[/dim]\n"
        )
        ...
        for i, layer in enumerate(layers):
            ...
            lines_per_box = 3
            row_boxes = [
                boxes[j : j + lines_per_box]
                for j in range(0, len(boxes), lines_per_box)
            ]
    ...
    def _render_html_topology(self, dot_graph: str, directory: str) -> bool:
        ...
        for src, tgt in edges:
            if src in node_idx and tgt in node_idx:
                vis_edges.append(
                    f'{{from: {node_idx[src]}, to: {node_idx[tgt]}, arrows: "to"}}'
                )
...
new vis.Network(container, data, options);
        ...
        try:
            ...
            for root, dirs, files in os.walk(directory):
                ...
                dirs[:] = [
                    d
                    for d in dirs
                    if d not in [".terraform", ".terragrunt-cache", ".git"]
                ]
                            ...
                            if "dependency" in parsed:
                                ...
                                for dep_block in parsed["dependency"]:
                                    for dep_name, dep_config in dep_block.items():
                                        config_path = (
                                            dep_config.get("config_path", [""])[0]
                                            if isinstance(
                                                dep_config.get("config_path"), list
                                            )
                                            else dep_config.get("config_path", "")
                                        )
                                        mock_outputs_list = dep_config.get(
                                            "mock_outputs", []
                                        )
                                        mock_outputs = (
                                            mock_outputs_list[0]
                                            if mock_outputs_list
                                            else {}
                                        )
                                        ...
                                        dependencies_info[stack_name]["dependencies"][
                                            dep_name
                                        ] = dep_info
    ...
    def _display_dependency_graph(
        ...
        self.console.print(
            Panel(
                "[bold blue]🔄 Enhanced Dependency Visualization 🔄[/bold blue]",
                box=box.ROUNDED,
            )
        )
    ...
    def _display_dependency_details(self, dep_info: Dict) -> None:
        ...
        self.console.print(
            Panel(
                "[bold cyan]📋 Dependency Details (from terragrunt.hcl)[/bold cyan]",
                box=box.ROUNDED,
            )
        )
        ...
        for stack_name, info in dep_info.items():
            if info["dependencies"]:
                ...
                for dep_name, dep_config in info["dependencies"].items():
                    ...
                    self.console.print(
                        f"     Path: [dim]{dep_config['config_path']}[/dim]"
                    )

                    # Display mock_outputs if available
                    if dep_config["mock_outputs"]:
                        self.console.print("     Mock Outputs:")
    ...
    def _display_risk_legend(self) -> None:
        ...
        legend.add_row(
            "[green]Low Risk (0-25%)[/green]", "Minimal risk of issues if changed"
        )
        legend.add_row(
            "[yellow]Medium Risk (26-50%)[/yellow]",
            "Moderate risk, changes should be reviewed",
        )
        legend.add_row(
            "[orange1]High Risk (51-75%)[/orange1]",
            "Significant risk, careful testing needed",
        )
        legend.add_row(
            "[red]Critical Risk (76-100%)[/red]",
            "Very high risk, changes may cause cascading issues",
        )

        self.console.print("\n[bold]Risk Level Legend:[/bold]")
        self.console.print(legend)

    def _parse_dot_graph(
        self, dot_graph: str
        ...
        for root in root_nodes:
            if root not in processed:
                ...
                self._build_tree_recursive(
                    root, edges, node_tree, processed, risks=risks
                )
    ...
    def _build_tree_recursive(
        ...
        for source, target in edges:
            if source == current_node:
                ...
                self._build_tree_recursive(
                    target, edges, child, processed, depth + 1, risks=risks
                )
    ...
    def _run_blast_radius_check(
        self, directory: str, recursive: bool = False, **kwargs
    ) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Blast radius: project_type={project_type}, live={live_mode}"
            )
            ...
            if not tfplan_files:
                self.ui.print_warning(
                    "No tfplan.json files found. Run 'terraform plan -out=tfplan && terraform show -json tfplan > tfplan.json' first."
                )
                return False
            ...
            if len(tfplan_files) > 1:
                self.ui.print_info(
                    f"Found {len(tfplan_files)} tfplan files, using: {plan_file}"
                )

            # Run blast radius assessment
            assessment = blast_service.assess_blast_radius(
                directory=directory, recursive=recursive, plan_file=plan_file
            )
    ...
    def _run_cfn_blast_radius(
        ...
        **kwargs,
        ...
        from ....services.check.project.cfn_blast_radius_service import (
        ...
        try:
            ...
            if project_type == "cdk":
                ...
                if not templates:
                    self.ui.print_warning(
                        "No CDK templates found in cdk.out/. Run 'cdk synth' first."
                    )
                    return False
            else:
            ...
            self.ui.print_info(
                f"Found {len(templates)} {'CDK' if project_type == 'cdk' else 'CloudFormation'} template(s)"
            )
            ...
            for template in templates:
                ...
                if live:
                    if not stack_name:
                        ...
                        stack_name_inferred = template_name.replace(
                            ".template", ""
                        ).replace("_", "-")
                        self.ui.print_info(
                            f"  Using inferred stack name: {stack_name_inferred}"
                        )
                    else:
            ...
            self.ui.print_error(
                f"Failed to assess CloudFormation blast radius: {str(e)}"
            )
            return False
    ...
    def _display_cfn_blast_radius(self, result) -> None:
        ...
        self.console.print(f"\n{'=' * 60}")
        self.console.print(
            f"💥 [{risk_color}]{result.risk_level}[/{risk_color}] — {result.stack_name} ({result.mode} mode)"
        )
        self.console.print(
            f"   Resources: {result.total_resources} total, {len(result.changed_resources)} affected ({result.blast_radius_percentage:.1f}%)"
        )
        ...
        if result.changed_resources:
            ...
            if len(result.changed_resources) > 15:
                table.add_row(
                    "...", f"+{len(result.changed_resources) - 15} more", "", ""
                )
    ...
    def _save_topology_report(self, directory: str) -> None:
        ...
        try:
            ...
            try:
                from ....services.document.architecture_renderer import (
                    render_architecture_diagram,
                )

                png_path = render_architecture_diagram(
                    topology, str(reports_dir), fmt="png"
                )
                if png_path:
    ...
    def _run_cost_analysis(
        ...
        try:
            ...
            from ....services.check.project.cost.unified_cost_report import (
            ...
            if not tfplan_files and not cf_templates:
                ...
                self.ui.print_info(
                    "For Terraform: Run 'terraform plan -out=tfplan && terraform show -json tfplan > tfplan.json'"
                )
                self.ui.print_info(
                    "For CloudFormation: Ensure .yaml, .yml, or .json template files are present"
                )
                return False
            ...
            if tfplan_files:
                for tfplan_file in tfplan_files:
                    ...
                    cost_results.append(
                        {"stack": Path(tfplan_file).parent.name, "analysis": analysis}
                    )
                    ...
                    json_path = (
                        reports_dir / f"cost_analysis_{plan_dir}_{timestamp}.json"
                    )
                    html_path = (
                        reports_dir / f"cost_analysis_{plan_dir}_{timestamp}.html"
                    )
            ...
            if cf_templates:
                for template in cf_templates[:3]:  # Limit to first 3 templates
                    ...
                    self._display_cost_analysis(analysis)
                    cost_results.append(
                        {"stack": Path(template).stem, "analysis": analysis}
                    )
                    ...
                    json_path = (
                        reports_dir / f"cost_analysis_{template_name}_{timestamp}.json"
                    )
                    html_path = (
                        reports_dir / f"cost_analysis_{template_name}_{timestamp}.html"
                    )
            ...
            if tfplan_files:
                reports_dir = Path("Reports") / "cost-analysis"
                index_path = unified_report.generate_unified_index(
                    reports_dir, "Infrastructure"
                )
    ...
    def _run_cfn_drift(
        ...
        **kwargs,
        ...
        """
        from ....services.check.project.drift.cfn_drift_service import (
            CfnDriftDetectionService,
        )

        region = kwargs.get("region") or os.environ.get(
        ...
        if project_type == "cdk":
            self.ui.print_info(
                f"🔍 Detecting drift for CDK project (region: {region})..."
            )
        else:
            self.ui.print_info(
                f"🔍 Detecting drift for CloudFormation project (region: {region})..."
            )
        ...
        if stack_names:
            ...
            self.ui.print_info(
                "  Static mode: comparing templates vs deployed state..."
            )
    ...
    def _run_drift_detection(
        ...
        try:
            ...
            from ....services.check.project.drift.drift_service import (
            ...
            if project_type in ("cloudformation", "cdk"):
                ...
                if not plan_files:
                    ...
                    if plan_dir:
                        ...
                        if plan_files:
                            self.ui.print_info(
                                f"Using plan files from --plan-file directory: {plan_dir}"
                            )
                ...
                if not plan_files and not recursive:
                    ...
                    for plan_dir in common_plan_dirs:
                        if os.path.isdir(plan_dir):
                            plan_files = self._find_tfplan_files(
                                plan_dir, recursive=True
                            )
                            if plan_files:
                                self.ui.print_info(
                                    f"Auto-detected plan files in {plan_dir}/"
                                )
                                break
                    ...
                    if not plan_files:
                        ...
                        has_terragrunt = any(
                            os.path.exists(os.path.join(directory, m))
                            for m in terragrunt_markers
                        )
                        if has_terragrunt or project_type in (
                            "terragrunt",
                            "terraform-terragrunt",
                        ):
                            self.ui.print_info(
                                "Terragrunt project detected, searching recursively for plan files..."
                            )
                            plan_files = self._find_tfplan_files(
                ...
                if plan_files:
                    self.ui.print_info(
                        f"Found {len(plan_files)} tfplan.json file(s), analysing for drift..."
                    )
                    summary = service.detect_drift(
                        directory,
                        recursive=True,
                    ...
                    self.ui.print_info(
                        f"No tfplan.json found. Running live {tftool} plan to detect drift..."
                    )
                    summary = service.detect_drift(
                        directory, recursive, filter_tags=filter_tags
                    )
            ...
            if evaluation.ignored_addresses:
                self.ui.print_info(
                    f"📋 Policy: {len(evaluation.ignored_addresses)} resource(s) ignored by .driftpolicy"
                )
            if evaluation.accepted_addresses:
                self.ui.print_info(
                    f"✅ Policy: {len(evaluation.accepted_addresses)} resource(s) auto-accepted by .driftpolicy"
                )
            ...
            override_map = {
                v.address: v.severity_override
                for v in evaluation.verdicts
                if v.severity_override
            }
            if override_map:
            ...
            if ai_provider and summary.has_drift:
                self.ui.print_info(
                    f"🤖 Running AI drift analysis (provider: {ai_provider})..."
                )
                from ....services.check.project.drift.drift_ai import (
    ...
    def _find_cloudformation_templates(
    ...
    def _is_cloudformation_template(
        self, file_path: str, indicators: List[str] = None
    ) -> bool:
    ...
    def _enforce_cost_policy(
        self, cost_results: list, policy_dir: str, directory: str
    ) -> bool:
        ...
        if not conftest:
            self.ui.print_warning(
                "conftest not found — skipping cost policy enforcement"
            )
            return True
        ...
        if policy_dir == "cost":
            ...
            if org_policy:
                ...
                if resolved:
                    ...
                    self.ui.print_warning(
                        "No 'cost' policy directory found in org policies"
                    )
                    return True
            else:
                ...
                if os.path.isdir(local_cost):
                    ...
                    self.ui.print_warning(
                        "No cost policy directory found. Create policy/cost/*.rego"
                    )
                    return True
        ...
        if not os.path.isdir(policy_dir):
            ...
            if resolved:
                ...
                self.ui.print_warning(
                    f"Could not resolve policy directory: {policy_dir}"
                )
                return True
        ...
        combined = {
            "summary": {
                ...
                "total_running_monthly_cost": sum(
                    r["analysis"].analysis_metadata.get(
                        "total_running_monthly_cost", r["analysis"].total_monthly_cost
                    )
                    for r in cost_results
                ),
        ...
        for r in cost_results:
            ...
            for svc, cost in analysis.cost_breakdown_by_service.items():
                combined["cost_by_service"][svc] = (
                    combined["cost_by_service"].get(svc, 0) + cost
                )
    ...
    def _display_cost_analysis(self, analysis) -> None:
        ...
        self.console.print(
            f"📅 [bold]Total Annual Cost:[/bold] [green]${analysis.total_annual_cost:.2f}[/green]"
        )
        ...
        if analysis.cost_breakdown_by_action:
            ...
            for action, cost in analysis.cost_breakdown_by_action.items():
                ...
                self.console.print(
                    f"  • {action}: [{color}]${cost:.2f}/month[/{color}]"
                )
        ...
        if analysis.resource_costs:
            ...
            for resource in analysis.resource_costs[:10]:  # Show first 10
                confidence_color = (
                    "green" if resource.confidence_level == "high" else "yellow"
                )
                self.console.print(f"  • {resource.resource_address}")
                self.console.print(
                    f"    Type: {resource.resource_type} | Cost: [green]${resource.monthly_cost:.2f}/month[/green] | Confidence: [{confidence_color}]{resource.confidence_level}[/{confidence_color}]"
                )
        ...
        api_status = (
            "✅ Online"
            if metadata.get("api_available")
            else "⚠️ Offline (using estimates)"
        )
        self.console.print(
    ...
    def _display_blast_radius_results(self, assessment) -> None:
        ...
        self.console.print("\n" + "=" * 80)
        self.console.print(
            "🎯 BLAST RADIUS ASSESSMENT (ITIL v4 Compliant)", style="bold cyan"
        )
        self.console.print("=" * 80)
...
[bold]Affected Components:[/bold] {len(assessment.affected_components)}
        ...
        self.console.print(
            Panel(
                summary_text.strip(), title="📊 Risk Summary", border_style=risk_color
            )
        )
        ...
        if assessment.recommendations:
            ...
            self.console.print(
                Panel(rec_text, title="📋 ITIL v4 Recommendations", border_style="blue")
            )
        ...
        if assessment.mitigation_steps:
            ...
            self.console.print(
                Panel(mit_text, title="🛡️ Risk Mitigation Steps", border_style="green")
            )
        ...
        if assessment.rollback_plan:
            ...
            self.console.print(
                Panel(rollback_text, title="🔄 Rollback Plan", border_style="orange3")
            )
    ...
    def _run_stack_optimizer(self, directory: str, **kwargs) -> bool:
        ...
        if not stacks_input:
            self.ui.print_error(
                "--stacks is required for stack-optimizer (comma-separated list)"
            )
            return False

        target_stacks = [
            s.strip() for s in re.split(r"[,\s]+", stacks_input) if s.strip()
        ]
...
cli = CheckIaCCommand.as_click_command(
    ...
    click.option(
        "--ai-provider",
        type=click.Choice(
            ["openai", "bedrock", "azure", "ollama"], case_sensitive=True
        ),
        default=None,
