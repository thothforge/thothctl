...
class ProjectCLI(click.MultiCommand):
    def list_commands(self, ctx: click.Context) -> list[str]:
        commands = []
        commands_path = Path(__file__).parent / "project"
        ...
        try:
            ...
            logger.error(f"Error listing project commands: {e}")
            return []
    ...
    def get_command(self, ctx: click.Context, cmd_name: str) -> Optional[click.Command]:
        try:
            ...
            # Import the module
            module_name = f"thothctl.commands.check.commands.project.{cmd_name}"
