...
class CheckProjectIaCCommand(ClickCommand):
    ...
    def validate(self, **kwargs) -> bool:
        ...
        if (
            kwargs.get("check_type")
            and kwargs["check_type"] not in self.supported_check_types
        ):
            self.logger.error(
        ...
        for line in lines:
            ...
            if "✅" in line or "❌" in line:
                ...
                if item_data:
                    ...
                    if (
                        " root exists! in " in line
                        or " exists!" in line
                        or (" doesn't exist in ." in line and "optional" in line)
                        or (" doesn't exist in ." in line and "optional" not in line)
                    ):
    ...
    def _parse_validation_line(self, line: str) -> dict:
        ...
        if (
            item.endswith(
                (".tf", ".hcl", ".yaml", ".yml", ".md", ".txt", ".json", ".toml")
            )
            or "." in item
        ):
            item_type = "📄"
    ...
    def _execute(self, **kwargs) -> None:
        ...
        try:
            ...
            org_violations = self._check_org_policy(
                directory,
                project_type,
                org_policy,
                skip_org=kwargs.get("skip_org_policy", False),
            )
            ...
            if org_violations is not None:
                ...
                mandatory_fails = [
                    v for v in org_violations if v.enforcement == "mandatory"
                ]
    ...
    def _check_org_policy(
        self, directory: str, project_type: str, org_policy=None, skip_org: bool = False
    ):
        """Check project against organizational policy if available."""
        if skip_org:
            self.console.print(
                "[dim]📜 Org policy check skipped (--skip-org-policy)[/dim]"
            )
            return None
        ...
        from .....services.check.rule_merger import (
            evaluate,
            load_org_rules,
            merge_with_project,
        )
    ...
    def _display_org_violations(self, violations, enforcement: str):
        ...
        if recommended:
            table = Table(
                title="⚠️ Recommendations", box=box.ROUNDED, header_style="bold yellow"
            )
            table.add_column("Rule", style="cyan")
            table.add_column("Expected", style="green")
    ...
    def _validate_project_structure(
        ...
        directory: str,
        mode: str = "soft",
        check_type: str = "structure",
        project_type: str = "stack",
    ) -> bool:
        ...
        return validate(
            directory=directory, check_type=validation_check_type, mode=mode
        )
...
cli = CheckProjectIaCCommand.as_click_command(
    ...
    click.option(
        ...
        type=click.Choice(
            ["structure", "metadata", "compliance"], case_sensitive=False
        ),
        default="structure",
    ),
    click.option(
