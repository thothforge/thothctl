        ...
        **kwargs,
        ...
        if template_url:
            ...
            self.ui.print_success(
                f"✅ Template URL set for {project_type}: {template_url}"
            )
        else:
            # Get template URL
            ...
            if url:
                ...
                self.ui.print_warning(
                    f"No custom template URL configured for {project_type}"
                )
                self.ui.print_info(
                    "💡 Use 'thothctl list templates' to see available templates"
                )


# Create the Click command
cli = TemplateConfigCommand.as_click_command(
    help="Configure custom template repository URLs"
)(
    click.option(
        "-pt",
        "--project-type",
