    ...
    def _execute(self, port, host, stdio):
        ...
        if stdio:
            ...
            with self.ui.status_spinner(f"Starting MCP server on {host}:{port}..."):
                ...
                self.ui.print_info(
                    f"Health check will be available at: http://{host}:{port}/health"
                )
                self.ui.print_info(
                    f"MCP endpoint will be available at: http://{host}:{port}/mcp/v1"
                )
                self.ui.print_info(
                    f"To stop the server, use: thothctl mcp stop --port {port}"
                )
