...
class MCPRegisterCommand(ClickCommand):
    ...
    def _execute(self, port, name, force, stdio, scope):
        ...
        with self.ui.status_spinner(
            f"Registering MCP server with Kiro CLI ({scope_label}) as '{name}'..."
        ):
            try:
                ...
                if os.path.exists(config_path):
                    try:
                        ...
                        self.ui.print_info(
                            f"Found existing configuration with {len(existing_config.get('mcpServers', {}))} server(s)"
                        )
                ...
                if name in existing_config["mcpServers"]:
                    if not force:
                        self.ui.print_warning(
                            f"Server '{name}' already exists in configuration"
                        )
                        if not self.ui.confirm(
                            f"Overwrite existing '{name}' server configuration?"
                        ):
                            self.ui.print_warning("Registration cancelled")
                            return
                    self.ui.print_info(
                        f"Overwriting existing '{name}' server configuration"
                    )
                ...
                self.ui.print_success(
                    f"Successfully registered '{name}' MCP server with Kiro CLI"
                )
                ...
                self.ui.print_error("Failed to register MCP server with Kiro CLI")
