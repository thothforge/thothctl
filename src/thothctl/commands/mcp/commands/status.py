                    ...
                    if process.is_running():
                        ...
                        if (
                            ...
                            self.ui.print_success(
                                f"MCP server process (PID {pid}) is running"
                            )
                        else:
                            self.ui.print_warning(
                                f"Process with PID {pid} is running but doesn't appear to be an MCP server"
                            )
                    else:
        ...
        try:
            with self.ui.status_spinner("Checking Amazon Q MCP registration..."):
                result = subprocess.run(
                    "q mcp list", shell=True, check=True, capture_output=True, text=True
                )
