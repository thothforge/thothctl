...
from thothctl.services.ai_review.main import _detect_mode, _dispatch, app
...
class TestDispatch:
    ...
    def test_dispatch_review(self, MockOrch):
        ...
        result = _dispatch(
            mode="review", directory="/tmp", provider="ollama", roles=["security"]
        )
...
class TestInvocations:
    ...
    def test_invocations_auto_detect_analyze(self, mock_dispatch):
        ...
        assert (
            call_kwargs.kwargs.get("mode") == "analyze"
            or call_kwargs[1].get("mode") == "analyze"
        )
    ...
    def test_invocations_explicit_mode(self, mock_dispatch):
        ...
        call_kwargs = mock_dispatch.call_args
        assert (
            call_kwargs.kwargs.get("mode") == "fix"
            or call_kwargs[1].get("mode") == "fix"
        )
    ...
    @patch(
        "thothctl.services.ai_review.main._dispatch", side_effect=RuntimeError("boom")
    )
