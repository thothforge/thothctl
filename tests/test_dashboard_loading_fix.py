...
class TestDashboardLoadingFix:
    ...
    @patch(
        "thothctl.services.dashboard.data_loader.DashboardDataLoader.get_inventory_data"
    )
        ...
        for script in scripts:
            if len(script) > 50:  # Only check substantial scripts
                assert script.count("{") == script.count(
                    "}"
                ), "Unbalanced curly braces in JS"
                assert script.count("(") == script.count(
                    ")"
                ), "Unbalanced parentheses in JS"
                assert script.count("[") == script.count(
                    "]"
                ), "Unbalanced square brackets in JS"
