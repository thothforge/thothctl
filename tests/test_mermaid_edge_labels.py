...
def test_create_mermaid_with_edge_labels(test_stack):
    ...
    assert (
        "security_groups" in mermaid_content
    )  # Note: hyphens replaced with underscores
...
def test_edge_label_matching():
    ...
    dep_info = {
        "security-groups": {
            "vpc": {
                "config_path": "../vpc",
                "mock_outputs": {"vpc_id": "vpc-123", "subnet_ids": ["subnet-1"]},
            }
        },
        "ec2": {
            "vpc": {"config_path": "../vpc", "mock_outputs": {"vpc_id": "vpc-123"}},
            "sg": {
                "config_path": "../security-groups",
                "mock_outputs": {"security_group_id": "sg-123"},
            },
        },
    }
    ...
    has_labeled_edge = any(
        "|" in line and ("vpc_id" in line or "security_group_id" in line)
        for line in edge_lines
    )
