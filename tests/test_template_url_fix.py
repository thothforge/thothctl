...
class TestUpgradeServiceTemplateUrl:
    ...
    def test_reads_template_url_when_present(
        self, upgrade_service, project_with_metadata
    ):
        ...
        with patch.object(
            upgrade_service, "_clone_template", side_effect=Exception("stop")
        ):
    ...
    def test_fails_when_neither_url_present(
        self, upgrade_service, project_with_metadata
    ):
...
class TestRepoMetaIncludesTemplateUrl:
    ...
    def test_azure_clone_repo_includes_template_url(self):
        ...
        fake_repos = [
            {
                "Name": "my-template",
                "RemoteUrl": "https://dev.azure.com/org/proj/_git/my-template",
            }
        ]

        with patch.object(
            azdo, "get_repos_patterns", return_value=fake_repos
        ), patch.object(
            azdo.inquirer, "prompt", return_value={"repository": "my-template"}
        ), patch.object(
            azdo.git.Repo, "clone_from", return_value=mock_repo
        ), patch.object(azdo.git.Repo, "init"), patch.object(
            azdo.shutil, "rmtree"
